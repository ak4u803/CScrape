...
class NeweggScraper(BaseScraper):
    ...
    def scrape_product(self, url: str) -> Optional[Dict[str, Any]]:
        """
        Scrape a single Newegg product page
        ...
        """
        self.logger.info(f"Scraping Newegg product: {url}")
        ...
        product_data = self.format_product_data(
            ...
            url=url,
            image_url=image_url,
            availability=availability if availability else 'In Stock'
        )
    ...
    def search_products(self, query: str, max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info(f"Searching Newegg for: {query}")
        ...
        product_items = soup.select('.item-cell, .item-container')
        ...
        for item in product_items[:max_results]:
            try:
                ...
                if title_elem.name == 'a':
                    ...
                    title = title_elem.get_text(strip=True)
                    url_elem = item.select_one('a.item-img, a.item-title')
                    product_url = url_elem.get('href', '') if url_elem else ''
                
                # Extract price
                price_elem = item.select_one('.price-current strong, .price-current')
                ...
                self.logger.error(f"Error parsing Newegg search result: {str(e)}")
                continue
        
        self.logger.info(f"Found {len(products)} products on Newegg")
        return products
