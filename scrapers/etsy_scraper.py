"""
Etsy-specific scraper implementation
"""
...
class EtsyScraper(BaseScraper):
    ...
    def scrape_product(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        """
        self.logger.info(f"Scraping Etsy product: {url}")
        ...
        product_data = self.format_product_data(
            ...
            url=url,
            image_url=image_url,
            availability=availability if availability else 'Made to order'
        )
    ...
    def search_products(self, query: str, max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info(f"Searching Etsy for: {query}")
        ...
        product_items = soup.select('[data-search-results-item], .v2-listing-card')
        
        for item in product_items[:max_results]:
            try:
                # Extract title and URL
                title_elem = item.select_one('.v2-listing-card__title, h3 a')
                if not title_elem:
                    continue
                ...
                if title_elem.name == 'a':
                    ...
                    title = title_elem.get_text(strip=True)
                    url_elem = item.select_one('a.listing-link')
                    product_url = url_elem.get('href', '') if url_elem else ''
                
                # Extract price
                price_elem = item.select_one('.currency-value, .n-listing-card__price')
                price_text = price_elem.get_text(strip=True) if price_elem else None
                
                # Extract image
                img_elem = item.select_one('.listing-card-image img, .wt-width-full img')
                ...
                self.logger.error(f"Error parsing Etsy search result: {str(e)}")
                continue
        
        self.logger.info(f"Found {len(products)} products on Etsy")
        return products
