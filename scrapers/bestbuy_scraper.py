"""
Best Buy-specific scraper implementation
"""
...
class BestBuyScraper(BaseScraper):
    ...
    def scrape_product(self, url: str) -> Optional[Dict[str, Any]]:
        """
        Scrape a single Best Buy product page
        ...
        """
        self.logger.info(f"Scraping Best Buy product: {url}")
    ...
    def search_products(self, query: str, max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        """
        self.logger.info(f"Searching Best Buy for: {query}")
        ...
        product_items = soup.select('.sku-item, .list-item')
        ...
        for item in product_items[:max_results]:
            try:
                ...
                # Extract price
                price_elem = item.select_one('.priceView-customer-price span[aria-hidden="true"], .priceView-hero-price span')
                ...
                self.logger.error(f"Error parsing Best Buy search result: {str(e)}")
                continue
        
        self.logger.info(f"Found {len(products)} products on Best Buy")
        return products
