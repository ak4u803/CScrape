...
class AmazonScraper(BaseScraper):
    ...
    def scrape_product(self, url: str) -> Optional[Dict[str, Any]]:
        ...
        """
        self.logger.info(f"Scraping Amazon product: {url}")
        ...
        # Handle alternative image URL (data-old-hires attribute)
        if not image_url or 'data:image' in image_url:
            image_url = self._extract_attribute(soup, self.selectors['image'], 'data-old-hires')
    ...
    def search_products(self, query: str, max_results: int = 10) -> List[Dict[str, Any]]:
        ...
        # Amazon has multiple possible selectors for product items
        product_items = soup.select('[data-component-type="s-search-result"]')
        
        for item in product_items[:max_results]:
            try:
                # Extract title and URL
                title_elem = item.select_one('h2 a, .a-link-normal.s-no-outline')
                if not title_elem:
                    continue
                ...
                self.logger.error(f"Error parsing Amazon search result: {str(e)}")
                continue
